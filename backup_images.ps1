...
foreach ($ext in $imageExtensions) {
    ...
    foreach ($file in $files) {
        try {
            ...
            Write-Host "  ✓ Copied: $relativePath ($([math]::Round($fileSize/1KB, 1))KB)" -ForegroundColor Gray
        } catch {
...
Write-Host "Total size: $([math]::Round($totalSize/1MB, 1))MB" -ForegroundColor Green
